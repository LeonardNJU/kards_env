...
class Player:
    def __init__(self, name: str) -> None:
        ...
        self.hand = Hand()
        self.fatigue = 0
        self.obj_on_field = []  # List to hold objects on the field, e.g., HQ, units, etc.
        self.kredits = 0  # Player's kredits, can be used to play cards or activate abilities
        self.kredits_slot = 0  # Kredits slot, used to track kredits spent or available for use
    ...
    def reschedule_phase(self, cards_to_draw: int = 0) -> None:
        ...
        if cards_to_redraw and cards_to_redraw[0] != -1:
            cards_to_redraw = [num for num in cards_to_redraw if len(cards_to_redraw) >= num >= 0][::-1]  # Reverse to remove from the end of the list first
